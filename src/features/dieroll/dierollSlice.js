...
export const saveDiePrefs = createAsyncThunk(
  "dieroll/savePrefs",
  async (newPrefs) => {
...
const dierollSlice = createSlice({
  ...
  reducers: {
    ...
    toggle20Mode: (state) => {
      state.twentyMode === "delay"
        ? (state.twentyMode = "toggle")
        : (state.twentyMode = "delay");
    },
    toggleNon20Mode: (state) => {
      state.nonTwentyMode === "delay"
        ? (state.nonTwentyMode = "toggle")
        : (state.nonTwentyMode = "delay");
    },
