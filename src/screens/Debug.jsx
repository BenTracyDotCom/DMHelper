  ...
  return (
    <View>
      <Modal animationType="slide" visible={show} presentationStyle="pageSheet">
        <Text className="w-11/12 h-11/12 m-auto">Current State:</Text>
        <ScrollView>
      ...
      <Text className="mx-auto mt-5 text-2xl">Screens</Text>
      <TouchableOpacity
        className="mx-auto mt-5 w-11/12 bg-teal-700 rounded-2xl"
        key="campaign"
        onPress={() =>
          navigation.navigate("Campaign", { name: state.campaign.title })
        }
      >
        <Text className="p-2 m-auto text-white">Campaign</Text>
      </TouchableOpacity>
      {Object.keys(pages).map((page) => (
        <TouchableOpacity
          ...
          key={pages[page]}
          onPress={() => {
            navigation.navigate(pages[page]);
          }}
        >
          <Text className="p-2 m-auto text-white">{page}</Text>
        </TouchableOpacity>
      ))}
      <TouchableOpacity
        onPress={() => setShow(true)}
        className="mx-auto my-5 rounded-2xl bg-slate-500"
      >
