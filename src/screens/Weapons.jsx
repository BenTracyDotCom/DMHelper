...
export default function Weapons() {
  ...
  const weaponCategories = [
    ...
    "equipment-categories/magic-weapon",
  ];
  ...
  useEffect(() => {
    const fetchWeapons = async () => {
      try {
        ...
        const promisesNormal = jsonNormal.equipment.map((weapon) =>
          fetch(`https://www.dnd5eapi.co${weapon.url}`).then((resp) =>
            resp.json(),
          ),
        );

        const responseMagic = await fetch(
          "https://www.dnd5eapi.co/api/magic-items",
        );
        const jsonMagic = await responseMagic.json();
        const promisesMagic = jsonMagic.results.map((item) =>
        ...
        const weaponDetailsMagic = allMagicItemDetails.filter(
          (item) =>
            item.equipment_category &&
            item.equipment_category.name === "Weapon",
        );

        const weaponDetailsNormalWithFlag = weaponDetailsNormal.map(
          (weapon) => ({ ...weapon, isMagicItem: false }),
        );
        const weaponDetailsMagicWithFlag = weaponDetailsMagic.map((weapon) => ({
          ...weapon,
          isMagicItem: true,
        }));

        const weaponDetails = [
          ...weaponDetailsNormalWithFlag,
          ...weaponDetailsMagicWithFlag,
        ];
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          const handlePress = item.isMagicItem
            ? handlePressMagic
            : handlePressNormal;
          return (
                ...
                <View style={styles.itemDetails}>
                  {item.isMagicItem ? (
                    <>
                      ...
                      <Text style={styles.itemDetail}>
                        Variant: {item.varient === true ? "yes" : "no"}
                      </Text>
                    </>
                  ) : (
