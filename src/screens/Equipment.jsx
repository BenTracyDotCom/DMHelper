    ...
    <ScrollView style={styles.scrollContainer}>
      ...
      <View style={styles.contentContainer}>
        {equipmentData.equipment_category && (
          ...
          >{`Category: ${equipmentData.equipment_category.name}`}</Text>
        )}
        {equipmentData.gear_category && (
          <Text
            style={styles.details}
          >{`Gear Category: ${equipmentData.gear_category.name}`}</Text>
        )}
        <Text
          style={styles.details}
        >{`Cost: ${equipmentData.cost.quantity} ${equipmentData.cost.unit}`}</Text>
        <Text style={styles.details}>{`Weight: ${equipmentData.weight}`}</Text>
        {equipmentData.desc && equipmentData.desc.length > 0 && (
          <Text style={styles.details}>{`${equipmentData.desc.join(
            ", ",
          )}`}</Text>
        )}
        {equipmentData.special && equipmentData.special.length > 0 && (
          <Text style={styles.details}>{`Special: ${equipmentData.special.join(
            ", ",
          )}`}</Text>
        )}
        {equipmentData.contents && equipmentData.contents.length > 0 && (
          <Text style={styles.details}>{`Contents: ${equipmentData.contents
            .map((content) => content.name)
            .join(", ")}`}</Text>
        )}
        {equipmentData.properties && equipmentData.properties.length > 0 && (
          <Text style={styles.details}>{`Properties: ${equipmentData.properties
